...
RUN git clone https://github.com/luizbills/gitpod-wordpress $HOME/gitpod-wordpress && \
    cat $HOME/gitpod-wordpress/.gitpod/.bashrc.sh >> $HOME/.bashrc && \
    . $HOME/.bashrc && \
...
RUN go get github.com/mailhog/MailHog && \
    ...
    echo "include $HOME/gitpod-wordpress/.gitpod/apache.conf" > /etc/apache2/apache2.conf && \
    echo ". $HOME/gitpod-wordpress/.gitpod/apache.env.sh" > /etc/apache2/envvars && \
    ### PHP ###
    ...
    cat /home/gitpod/gitpod-wordpress/.gitpod/php.ini >> /etc/php/${PHP_VERSION}/apache2/php.ini && \
    ### Setup PHP in Apache ###
    a2dismod php* && \
    a2dismod mpm_* && \
    a2enmod mpm_prefork && \
